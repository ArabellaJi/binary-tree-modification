 ...
 *
 * The header is read from disk by the constructor, and rewritten to
 * the file whenever it is changed.
 *
 */
...
class BTreeFile::PhysicalBlock {
 ...
 private:
  ...
  static const unsigned KEY_LENGTH = MAX_KEY_LENGTH + 1;
  static const unsigned VALUE_LENGTH = MAX_VALUE_LENGTH + 1;
  static const unsigned PHYSICAL_BLOCK_SIZE = 512;

  PhysicalBlock() : _numberOfKeys(0)  {
    for (int i = 0; i < DEGREE; i ++) {
      _childData[i] = 0;
    }
    for (unsigned i = 0; i < KEY_LENGTH * (DEGREE - 1); i ++) {
      _keyData[i] = '\0';
    }
    for (unsigned i = 0; i < VALUE_LENGTH * (DEGREE - 1); i ++) {
      _valueData[i] = '\0';
    }
    for (unsigned i = 0; i < sizeof(_filler); i ++) {
      _filler[i] = '\0';
    }
  }
...
void BTreeFile::setRoot(BlockNumber root) {
  _root = root;
  updateHeader();  // Reflect the change in the header on disk
}
...
void BTreeFile::printStats() const {
